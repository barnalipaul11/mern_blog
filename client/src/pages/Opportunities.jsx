...
//             <OpportunityCard
//               key={item.id ?? `opportunity-${index}`}  // fallback to index if id missing
//               opportunity={item}
//             />
//           ))}
//         </div>
...
const Opportunities = () => {
  ...
  const filterAndSearch = (filters, query) => {
    ...
    // Fixed company filter - now properly handles both ID and name filtering
    if (filters.company) {
      results = results.filter(item => {
        // Check if filtering by company ID
        if (item?.company?.id === filters.company) return true;
        // Check if filtering by company name (case-insensitive)
        if (item?.company?.name?.toLowerCase() === filters.company.toLowerCase()) return true;
        // Check if the companyId field matches (fallback for different data structures)
        if (item?.companyId === filters.company) return true;
        return false;
      });
    }
    ...
    if (query) {
      ...
      results = results.filter(item =>
        ...
        item?.description?.toLowerCase().includes(lowerQuery) ||
        item?.company?.name?.toLowerCase().includes(lowerQuery) ||
        item?.companyId?.toLowerCase().includes(lowerQuery) ||
        item?.location?.toLowerCase().includes(lowerQuery)
      );
    }
    ...
    <div>
      ...
      {filteredOpportunities.length > 0 ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredOpportunities.map((item, index) => (
            <OpportunityCard
              key={item.id ?? `opportunity-${index}`}  // fixed template literal syntax
              opportunity={item}
            />
          ))}
        </div>
